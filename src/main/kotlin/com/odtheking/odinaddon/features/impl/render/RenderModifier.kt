...
object RenderModifier: Module(
    ...
    private val color by ColorSetting("Color", Colors.MINECRAFT_RED, true, desc = "The color of the block overlay.").withDependency { blockOverlay }
    private val renderStyle by SelectorSetting("Render Style", "Outline", listOf("Filled", "Outline", "Filled Outline"), desc = "Style of the box.").withDependency { blockOverlay }


    init {
        on<BlockOutlineEvent> {
            if (!blockOverlay) return@on
            val pos = blockOutline.blockPos()
            val aabb = pos.getBlockBounds()?.move(pos) ?: AABB(pos)
